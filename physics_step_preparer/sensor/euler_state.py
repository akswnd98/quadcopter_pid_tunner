...
class ZeroInitialization (EulerStateInitialization):
  ...
  def generate_init_state (self):
    return {
      'phi': np.array([0] * self.env_num, dtype=np.float64),
      'theta': np.array([0] * self.env_num, dtype=np.float64),
      'phi_dot': np.array([0] * self.env_num, dtype=np.float64),
      'theta_dot': np.array([0] * self.env_num, dtype=np.float64),
      'psi_dot': np.array([0] * self.env_num, dtype=np.float64)
    }

class EulerState (PhysicsStepPreparer):
